...
UDetector::UDetector(const char* template_model_path, const char* search_model_path,
    ...
    for (size_t scale_idx = 0; scale_idx < pyramid.size(); scale_idx++) {
        ...
        for (int x_start : x_starts) {
            for (int y_start : y_starts) {
                ...
                std::vector<MATCH_RESULT> pred_bboxes = umatcher->Match(window, threshold);
