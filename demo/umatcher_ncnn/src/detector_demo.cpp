...
void printUsage(const char* programName) {
    std::cout << "Usage: " << programName << " [options]\n"
              << "Options:\n"
              << "  --template_branch <path>   Path to template branch model (default: data/umatcher_ncnn/template_branch)\n"
              << "  --search_branch <path>     Path to search branch model (default: data/umatcher_ncnn/search_branch)\n"
...
int main(int argc, char* argv[]) {
    ...
    UDetector detector(
        (template_branch + ".param").c_str(),
        (template_branch + ".bin").c_str(),
        (search_branch + ".param").c_str(),
        (search_branch + ".bin").c_str(),
        128, 2.0,
        256, 4.0,
        16, 128,
        false
